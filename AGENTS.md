...
#### 1.0 前提ツール
Mobile MCP を使ったテストでは、**adb が利用可能であることを前提条件**とする。

- Android SDK Platform Tools をインストールする
- `adb` コマンドに PATH が通っていることを確認する
- `adb version` が実行できることを確認する
...
- `adb` がインストール済みで、`adb version` が実行できるか
- `adb devices` で対象デバイスが表示されるか

**確認せずにテストを開始してはいけません。**

#### 1.2 スクリーンショットと座標のキャリブレーション
テスト開始時に、**最初のスクリーンショットを撮影**し、あわせて `mobile_get_screen_size` を実行して画面サイズを取得すること。さらに、**座標を使う操作の前提確認として `uiautomator` で UI 階層を取得し、対象要素の bounds から座標根拠を確認すること。**
そのうえで、以下をユーザーに確認すること：

- スクリーンショットに表示されている画面が実際のデバイス画面と一致するか
- スクリーンショット上の座標とデバイス上の実際の座標がマッチしているか
- `mobile_get_screen_size` の結果と実機 / エミュレータの画面サイズが一致しているか
- `uiautomator` で取得した対象要素の位置・サイズが、実際に操作したい UI と一致しているか

**座標のずれはテスト失敗の最大原因です。スクリーンショットだけで座標を決めず、必ず `uiautomator` の UI 階層でも裏取りしてください。**
...
1. **操作前**: スクリーンショットを撮影し、現在画面を記録する
2. **UI階層確認**: `uiautomator` で UI 階層を取得し、タップ対象の要素を特定して bounds から座標を決める
3. **サイズ確認**: `mobile_get_screen_size` の結果を前提に、その座標が現在の画面サイズに対して妥当か確認する
4. **操作実行**: `uiautomator` で確認した座標でタップする
5. **操作後**: スクリーンショットを撮影し、意図した要素が操作されたことを確認する
6. **不一致時**: 想定と異なる場合は、スクリーンショットだけで再判断せず、再度 `uiautomator` と `mobile_get_screen_size` を取り直して現在のUI階層と座標を確認してから次の操作に進む

**キャッシュした座標を使い回さないこと。画面状態が変わるたびに `uiautomator` と `mobile_get_screen_size` を取り直し、最新状態の UI 階層を根拠に座標を再確認すること。スクリーンショットは補助証跡であり、座標特定の唯一の根拠にしてはいけない。**
...
2. **`uiautomator` で UI 階層を再取得し、対象要素の bounds を確認する**
3. **`mobile_get_screen_size` を再取得し、UI階層上の座標と画面サイズの整合性を再チェック**する
4. エラーの原因を特定する（座標ずれ / 画面遷移の失敗 / アプリのクラッシュ / タイミングの問題）
5. 必要に応じてスクリーンショット・UI階層・画面サイズを取り直し、現在の正確な画面状態を把握してからリトライする

**エラー発生時に `uiautomator` によるUI階層確認を省略してはいけません。スクリーンショットだけで原因を判断することを禁止します。**

### 4. Mobile MCP操作のベストプラクティス

- **タップ操作**: タップは座標指定で実施する前提とし、毎回 `uiautomator` で最新の UI 階層を取得して対象要素の bounds から座標を決めてから操作する
- **座標確認**: 座標タップを使う場合は毎回 `uiautomator` と `mobile_get_screen_size` を確認し、画面サイズに対して座標が妥当か検証する
...
- **ディープリンク**: ディープリンクを開く際は `adb shell am start -a android.intent.action.VIEW -d "e2etestflutter://..."` を利用して実行する
...
正常系E2E手順書は `e2etest/howto/README.md` を**唯一の親ファイル**として扱うこと。

- ユーザーが更新するのは原則として `e2etest/howto/README.md` のみとする
- AIは `e2etest/howto/README.md` に載っているシナリオ一覧だけを正とみなし、個別手順書を `e2etest/howto/*.md` に作成・更新する
- AIは、親ファイルに存在しない新規シナリオや補助手順書を勝手に増やしてはいけない
- AIは、親ファイルのシナリオ名・順序・リンク先ファイル名に合わせて個別手順書を同期すること
- AIは、共通前提・共通データ・結果トラッキングを親ファイルに集約し、個別手順書にはシナリオ固有の手順だけを書くこと
- テスト実施結果の更新対象はまず親ファイルとし、個別手順書を結果記録の主置き場にしてはいけない
- AIは、個々のテストを実行したら、その都度 `e2etest/howto/README.md` に結果を必ずフィードバックすること
- AIは、各シナリオの合格/不合格/保留を `e2etest/howto/README.md` のチェックリストと `結果メモ` に反映し、未反映のまま終了してはいけない
- AIは、テスト完了前に `e2etest/howto/README.md` の結果記録とチェックリストを見直し、実施内容と記録内容が一致していることを確認すること
- 親ファイルと個別手順書に差分がある場合、AIは親ファイルを優先して整合させること
- 既存の個別手順書が親ファイルから外れている場合は、即削除せずユーザーに確認すること。ただし、明示的に移行作業を依頼されている場合はこの限りではない
