...
テスト開始時に、**最初のスクリーンショットを撮影**し、あわせて `mobile_get_screen_size` を実行して画面サイズを取得したうえで、以下をユーザーに確認すること：
...
2. **サイズ確認**: `mobile_get_screen_size` の結果を前提に、その座標が現在の画面サイズに対して妥当か確認する
3. **操作実行**: 確認した座標でタップする
4. **操作後**: スクリーンショットを撮影し、意図した要素が操作されたことを確認する
5. **不一致時**: 想定と異なる場合は、再度スクリーンショットと `mobile_get_screen_size` で現在の画面状態を確認してから次の操作に進む

**キャッシュした座標を使い回さないこと。画面状態が変わるたびに `mobile_get_screen_size` と最新スクリーンショットで座標を再確認すること。**
...
2. **スクリーンショットと `mobile_get_screen_size` を用いて座標の整合性を再チェック**する（座標ずれが原因でないか確認）
3. エラーの原因を特定する（座標ずれ / 画面遷移の失敗 / アプリのクラッシュ / タイミングの問題）
4. 必要に応じてスクリーンショットと画面サイズを取り直し、現在の正確な画面状態を把握してからリトライする

**エラー発生時にスクリーンショットと座標の確認を省略してはいけません。これは毎回必ず行うルールです。**

### 4. Mobile MCP操作のベストプラクティス

- **タップ操作**: タップは座標指定で実施する前提とし、毎回最新のスクリーンショットと `mobile_get_screen_size` をもとに座標を確認してから操作する
- **座標確認**: 座標タップを使う場合は毎回 `mobile_get_screen_size` を確認し、画面サイズに対して座標が妥当か検証する
